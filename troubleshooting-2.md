...
``` r
### In this case, the errors are using load.packages() instead of library() and also we installed the singer package but not loading it###
library(dslabs)
```
...
``` r
glimpse(movieLens) # Take a look at tibble
```
...
``` r
### Changed year > 2000 in line 55 and year == 2000 in line 56###
...
``` r
### Used = to assign rather than == (which is for equal) ###
movieLens <- movieLens %>%
...
``` r
### Removed the c() since you dont need to make it into vector or list ###
movieLens %>%
...
``` r
### Group titles first then summarize ###
movieLens %>%
  group_by(title) %>%
...
``` r
### Made height and mass into vector ###
starWars %>%
  group_by(species) %>%
...
``` r
### Added comma after birth_location and added "" in the birth locations ###
...
fakeStarWars %>% mutate(birth_weight = (birth_year-1995)*0.45)
```
